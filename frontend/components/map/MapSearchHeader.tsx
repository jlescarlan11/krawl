...
export function MapSearchHeader({ onSearch, onCategorySelect }: MapSearchHeaderProps) {
  ...
  return (
    <header 
      className={`
        absolute top-0 right-0 z-[1000] h-[72px]
        transition-all duration-300 ease-in-out
        ${isExpanded ? 'left-0 md:left-80' : 'left-0 md:left-16'}
      `}
    >
      <div className="flex items-center justify-between gap-4 px-4 h-full">
        {/* Left Section: Search Bar + Category Pills */}
        ...
        {/* Right Section: Profile Avatar / Login Button */}
        <Link
